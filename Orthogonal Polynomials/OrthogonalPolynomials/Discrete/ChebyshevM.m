...
BeginPackage["OrthogonalPolynomials`Discrete`ChebyshevM`"]

aChebyshevM::usage="aChebyshevM[n,M,x,ops] returns value of Chebyshev discrete polynomial \
...
aChebyshevM::supp="Supporting set is the set of natural numbers {0,...,M}.";
aChebyshevM::dist="Distribution function is step function having jumps 1 \
    at points 0,...,M.";
aChebyshevM::bdpmt="Parameter of the family must be positive integer.";
(*aChebyshevM::acont="Measure does not have absolutly continuous part.";*)
Options[aChebyshevM]={WorkingPrecision->$MachinePrecision,OrthogonalPolynomials`ReturnList->False,
    Numerator->0};
Attributes[aChebyshevM]={ReadProtected};

Begin["`Private`"]

aChebyshevM[request_,M_]:=Module[{i},
    If[IntegerQ[M] && (M<=0),Message[aChebyshevM::bdpmt];Abort[];];
    If[request==="supp",
        Message[aChebyshevM::supp];
...
aChebyshevM[Order_?IntegerQ/;Order>-1,M_,x_,Ops___]:=
    ...
    {wrprec,list,num}={WorkingPrecision,OrthogonalPolynomials`ReturnList,
        Numerator}/.{Ops}/.Options[aChebyshevM];
